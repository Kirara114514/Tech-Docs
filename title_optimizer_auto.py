...
def main():
    ...
    print(f"找到 {len(docs_to_optimize)} 个需要优化的文档")
    ...
    # 显示前10个优化示例
    print("\n=== 优化示例（前10个）===")
    for i, doc in enumerate(docs_to_optimize[:10], 1):
    ...
    for doc in docs_to_optimize:
        try:
            # 检查新文件是否已存在
            if os.path.exists(doc['new_path']):
                print(f"⚠ 跳过: {os.path.basename(doc['old_path'])} -> 目标文件已存在")
                skipped_count += 1
                continue
    ...
    print(f"跳过: {skipped_count} 个文档（目标文件已存在）")
    print(f"备份位置: {backup_dir}")
    
    # 生成优化报告
    report_path = os.path.join(root_dir, '标题优化报告.md')
    with open(report_path, 'w', encoding='utf-8') as f:
        f.write("# 文档标题优化报告\n\n")
        f.write(f"## 统计信息\n")
        f.write(f"- 总文档数: {len(docs_to_optimize)}\n")
        f.write(f"- 成功优化: {success_count}\n")
        f.write(f"- 优化失败: {error_count}\n")
        f.write(f"- 跳过优化: {skipped_count}\n")
        f.write(f"- 备份位置: {backup_dir}\n\n")
        
        f.write("## 优化详情\n\n")
        for doc in docs_to_optimize:
            status = "成功" if os.path.exists(doc['new_path']) else "失败"
            f.write(f"### {doc['category']}/{os.path.basename(doc['old_path'])}\n")
            f.write(f"- 原因: {doc['reason']}\n")
            f.write(f"- 当前标题: {doc['old_title']}\n")
            if doc['real_title']:
                f.write(f"- 内容标题: {doc['real_title']}\n")
            f.write(f"- 新标题: {doc['new_title']}\n")
            f.write(f"- 状态: {status}\n\n")
