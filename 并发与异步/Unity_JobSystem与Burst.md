...
Unity Job System 与 Burst 是 Unity 高性能计算体系中的核心组合，适合大量连续数据、纯计算、可并行、低托管依赖的任务。Job System 负责以受约束的数据访问模型表达并行任务，Burst 负责将符合约束的计算代码编译为针对目标 CPU 优化的原生代码。理解这套体系的关键，不是把它当作普通异步工具或通用 C# 加速器，而是从数据布局、NativeContainer 生命周期、JobHandle 依赖、Complete 时机、Burst 兼容边界和 Profiler 验收出发，判断某个计算问题是否真的适合进入 Job 化路径。
...
在工程实践中，Job System 和 Burst 最容易被误用为“把代码放到子线程”或“给任意 C# 逻辑加速”的工具。这个理解会遮蔽真正的设计重点：Job 的收益不只来自多线程，更来自连续数据、依赖图、低 GC、受控内存访问和安全系统；Burst 的收益也不来自对复杂托管业务的魔法优化，而来自对数学密集、无托管引用、访问模式稳定的代码进行原生化与向量化优化。
...
- **版本：** v2.0
- **分类：** 并发与异步
- **标签：** Unity, C#, 性能优化, 工程治理, 架构设计, ECS, Job System, 工程实践
