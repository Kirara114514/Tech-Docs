...
协程与 UniTask 不是简单的新旧替代关系，而是适用于不同复杂度和不同架构层级的异步工具。协程、原生 Task、UniTask 和 Job System 应放在同一套维度下比较：调度模型、返回值、异常传播、取消机制、生命周期、GC 分配、线程语义、主线程安全、组合能力、调试方式和团队落地成本。简单主线程时序可以继续使用协程；复杂业务异步、返回值组合、取消超时和生命周期治理更适合 UniTask；后台纯 C# 工作可以使用 Task 或线程池；大规模数据并行计算则应评估 Job System 与 Burst。
...
在工程讨论中，这个问题容易被口号化：有人把 UniTask 视为协程的完全替代，也有人把 Task 简单等同于线程问题。更严谨的方式，是先拆清协程、Task、UniTask、Job System 与真正多线程之间的边界，再根据是否需要返回值、取消、异常传播、组合、主线程访问和高频性能来选型。
...
- **版本：** v2.0
- **分类：** 并发与异步
- **标签：** Unity, C#, 协程编程, UniTask, 异步编程, 性能优化, 架构设计, 工程实践
