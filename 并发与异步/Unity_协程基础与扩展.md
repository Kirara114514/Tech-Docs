...
Unity 协程不是线程，也不是普通 C# 的全局异步能力，而是由 Unity PlayerLoop 驱动的主线程分帧流程工具。理解协程，应从 C# 迭代器状态机、MonoBehaviour 宿主、yield 指令语义、生命周期停止规则、异常传播、GC 分配和工程封装边界出发。它适合 UI、动画、加载、表现流程和轻量分帧控制；不适合 CPU 密集计算、复杂业务返回值、强异常传播和大规模并发调度。
...
在团队知识库中，协程最容易被讲成“像同步代码一样写异步”的语法技巧，但这种说法会掩盖关键风险：协程第一次 yield 之前仍会阻塞当前帧；禁用 MonoBehaviour 与停用 GameObject 对协程的影响并不等同；全局 CoroutineRunner 会削弱自然生命周期；异常很难像 await 一样向调用方传播。正式文档必须把这些边界讲清楚，避免开发者把协程误用为通用异步框架。
...
协程的生命周期规则看似简单，实际很容易在项目里被误用。禁用 `MonoBehaviour.enabled` 不应被当成停止协程的可靠机制；停用承载协程的 GameObject、销毁宿主、显式 StopCoroutine、协程自然完成和 `yield break` 都属于不同路径。团队规范中应把“谁启动、谁停止、何时停止”写成代码结构，而不是依赖开发者记住某个隐式行为。对于 UI 面板、临时表现对象和场景域任务，保存协程句柄并在明确生命周期回调中停止，通常比依赖引擎隐式停止更可维护。

#### 2. 全局 Runner 是工具，不是生命周期替代品

全局 CoroutineRunner 能解决普通类不能直接 StartCoroutine 的问题，但它也会带来新的风险。一旦所有协程都挂到全局对象上，协程就不再自然跟随界面、场景或业务 owner 结束。正式项目中，全局 Runner 至少要要求调用方提供 owner、任务名和停止策略；更进一步，还应支持按 owner 停止、按场景域清理、异常统一捕获和调试面板。没有 owner 的全局协程，本质上就是无主异步流程，很容易在对象销毁后继续回调旧状态。

#### 3. 协程异常需要局部治理

协程异常不适合承担复杂错误传播。外部调用者无法像 await 一样自然捕获后续帧中的异常，因此关键协程应在内部或 runner 层增加保护。保护层至少要记录协程名、owner、当前阶段和异常堆栈。对于业务失败，例如资源不存在、网络返回业务错误或用户取消，不应全部依赖异常；更适合通过回调结果、状态对象或迁移到 UniTask 的 Result 模型表达。协程适合表现流程，复杂业务错误模型应交给更结构化的异步方案。

#### 4. 性能问题来自调用频率和对象分配

协程本身并非性能禁区。少量 UI 动画、引导流程、加载等待中的协程通常成本很低。真正需要警惕的是高频创建短协程、在列表项或战斗实体中大量启动协程、每次等待都创建新的 yield 对象、在 WaitUntil 条件里使用闭包或复杂逻辑，以及在协程第一帧执行大量同步计算。优化时应先看 Profiler 的 GC Alloc、调用次数和主线程耗时，再决定是否缓存 yield 指令、改成手写 Update 状态机、迁移 UniTask，或进入 Job System。

#### 5. 从协程迁移到 UniTask 要保持原语义

协程迁移不能只把 `yield return` 改成 `await`。必须检查原协程是否依赖 GameObject 停用自动停止、是否依赖 timeScale、是否等待 FixedUpdate 或 EndOfFrame、是否通过字段或回调传递结果、是否通过 StopCoroutine 阻止重复流程。迁移后需要提供等价 CancellationToken、PlayerLoopTiming、旧结果防覆盖和异常策略。否则语法变现代了，行为却可能与旧系统不一致。

#### 6. 评审重点应落在工程边界

协程代码进入公共框架层前，应能回答几个问题：宿主是谁，停止点在哪里，是否可能重复启动，是否有异常保护，是否产生高频分配，是否会在对象失效后写回，是否需要返回值，是否需要超时，是否更适合 UniTask 或状态机。对于功能代码，也应说明执行频率、预期数据规模、失败后的降级方式和是否需要真机验证。协程的可维护性来自这些边界，而不是来自某个 API 本身。
...
- **版本：** v2.0
- **分类：** 并发与异步
- **标签：** Unity, C#, 性能优化, 异步编程, 工程治理, 响应式编程, UniRx, 协程优化, 工程实践, 生命周期
