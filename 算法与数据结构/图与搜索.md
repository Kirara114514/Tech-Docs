...
## 摘要
图是最通用、表达能力最强的数据结构之一，用于描述现实世界中的复杂网状关系。本文系统介绍图的表示方法（邻接矩阵/邻接表）、遍历算法（DFS/BFS）以及经典图算法（最短路径、最小生成树、拓扑排序），并结合 Unity 开发中的实际场景展示图的应用。

## 正文

### 背景
前几篇探索了线性结构和树形结构，但它们不足以描述现实世界中更复杂的网状关系。图（Graph）正是用来描绘和解决这类复杂关系的利器。

图无处不在——社交网络中的好友关系、导航系统的最短路径、游戏地图中的 AI 寻路、构建系统的依赖管理，背后都是图的抽象模型。理解图的表示、遍历和核心算法，能将看似无序的复杂关系转化为结构化的可计算问题。
...
### 实现方案
1. **表示选型**：稠密图用邻接矩阵（O(1)查边），稀疏图用邻接表（省空间）
2. **遍历选型**：DFS 适合路径搜索和连通性检查，BFS 适合最短路径（无权重）
3. **Unity应用**：A*（启发式搜索）用于寻路，Dijkstra（带权BFS）用于路径规划
4. **拓扑排序**：处理依赖关系和构建系统时使用 Kahn 算法或 DFS 后序

### 总结
图是最通用的数据结构，从社交网络到游戏寻路无处不在。掌握图的表示（邻接表/矩阵）、遍历（DFS/BFS）和经典算法（最短路径、最小生成树、拓扑排序），是解决复杂问题的关键能力。图的思维方式——将问题建模为节点和边的关系——本身就是一种重要的抽象方法。
