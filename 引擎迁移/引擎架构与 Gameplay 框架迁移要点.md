...
#### 一、是否保留 Unity 式业务框架，应由边界兼容性决定
很多团队迁移时会希望保留原 Unity 项目的业务框架，例如事件总线、UIManager、资源管理器、战斗状态机、服务定位器或自研 ECS。保留旧框架并非一定错误，因为其中可能沉淀了项目规则和团队经验。但保留的前提是它能与 UE 的 World、GameInstance、Subsystem、Controller、State、Actor 生命周期和资源系统兼容。如果旧框架要求绕开这些边界，它就会从经验资产变成迁移阻力。

评估旧框架是否可保留，可以看三个问题。第一，它是否能接受 UE 的生命周期，而不是强行维护一套平行世界。第二，它是否能与蓝图、DataAsset、编辑器工作流和资产引用协作。第三，它是否能在多人 Authority 和复制模型下解释状态归属。如果这三个问题回答不了，就不应直接移植旧框架，而应拆出其中真正有价值的规则，再用 UE 原生结构重新落位。

#### 二、UE 原生框架也不应被迷信
强调 Gameplay Framework，并不意味着所有项目都要机械套满 GameMode、GameState、PlayerState、Subsystem、Component 和 DataAsset。小型单机项目、纯工具项目、非典型交互体验和强自研框架项目，可能只需要其中一部分。真正重要的是理解每个框架类解决的问题，而不是为了“看起来 UE 正统”强行增加层级。

例如一个离线编辑器工具可能不需要复杂 PlayerState；一个纯单机线性体验可能暂时不需要完整复制模型；一个小型原型可以先用较少层级验证玩法。但这些简化应是有意识的取舍，而不是因为团队不了解框架边界。成熟架构的标志不是层级最多，而是每一层都有理由。

#### 三、框架边界要允许内容团队安全扩展
Gameplay 架构如果只从程序角度看，会很容易把所有重要逻辑锁在 C++ 中，蓝图和 DataAsset 只剩少量参数。这种方式代码上安全，但内容生产效率可能很低。UE 的优势之一，正是让 C++ 定义稳定底座，让蓝图和资产提供可控扩展。迁移时应把“内容团队能否安全使用”作为框架验收项。

安全扩展意味着：蓝图可覆盖点少而明确，属性默认值合理，错误配置能被提示，DataAsset 有校验，组件能在编辑器中解释用途，关卡放置对象能显示调试信息。只有这样，框架才不只是程序内部结构，而是团队生产系统。

#### 四、框架迁移需要反例驱动
抽象讲解 GameMode、Controller、Subsystem 的职责很有必要，但反例更能让团队形成判断。例如把比分放在 GameMode 导致客户端读不到，把玩家进度放在 Pawn 导致死亡重生丢失，把 UI 直接引用 Character 导致观战和切换 Pawn 出问题，把规则写在 Level Blueprint 导致地图复用困难。这些反例应进入知识库和评审清单。

反例的价值在于让迁移团队看到旧习惯的后果。只有能识别错误落位，团队才真正掌握了正确落位。
...
- **版本：** v2.0
- **分类：** 引擎迁移
- **标签：** Unity, Unreal Engine, 引擎迁移, C#, Gameplay框架, 架构设计, 学习路线, 工程实践
