# Unity 中常见的 GC Alloc 场景与分析工具

## 摘要

本文主题是 Unity 项目中 **GC Alloc 的典型产生场景与分析工具链**。文章从工程实践层面系统梳理最常见、最隐蔽、最容易在版本迭代中反复出现的托管堆分配来源，并建立一套从发现问题、定位调用栈、确认对象类型、判断影响范围，到形成优化结论的完整分析流程。
...
第四，Unity Editor 中的分配数据不能完全代表真机或 Player。编辑器环境会引入检查器、序列化、编辑器回调、域重载、EditorOnly 逻辑、调试符号和额外工具开销。Profiler 数据应尽量在目标平台的 Development Build 或接近发布环境的 Player 中采集，因为 Editor 与构建后的 Player 行为可能不同。比如某些 API 在 Editor 中会出现额外分配，但在构建版本中不一定存在。因此，工程化的 GC 分析必须区分“编辑器开发体验问题”和“玩家侧性能问题”。
...
GC Alloc 分析的设计重点，是把“分配来源”与“场景影响”连接起来。只知道某个 API 可能分配并不够，还必须知道它是否发生在热路径、是否持续、是否影响帧预算、是否造成堆增长，以及是否能通过工具复现。正式项目应把 GC Alloc 分析设计成标准流程：场景分级、预算制定、Profiler 定位、Memory Profiler 引用链验证、自动化采样和回归报告。
...
1. Profiler、Memory Profiler、ProfilerRecorder 和 Profile Analyzer 的可用指标、采样开销和界面入口会随 Unity 版本变化，需要按项目实际版本确认。
2. Editor、Development Build、Release Player、Mono 与 IL2CPP 的分配表现可能不同，本文中的排查流程需要在目标平台上复测。
3. Addressables、YooAsset、自研资源系统和原生插件内存不会完全体现在 GC Alloc 中，需要结合项目资源体系补充专门分析规范。
...
- **版本：** v2.0
- **分类：** 内存管理
- **标签：** Unity, C#, 性能优化, 内存管理, 异步编程, 工程治理, 响应式编程, UniRx, GC, 缓存策略, 协程优化, 性能分析, Memory Profiler, 托管堆, 内存泄漏, 异常处理, 自动回收, 字符串, 字符串优化, 对象池, 集合复用, 工程实践, 生命周期
