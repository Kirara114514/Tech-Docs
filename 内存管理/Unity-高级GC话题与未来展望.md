...
本文讨论当常规 GC Alloc 优化已经完成之后，Unity 项目还可以从哪些更高层次继续治理内存、降低卡顿风险并提升运行时性能。文章围绕 ScriptableObject、静态数据、IL2CPP、C# Job System、Burst Compiler、Native Container、Addressables 以及运行时技术演进展开，重点不在罗列概念，而在说明这些技术分别解决哪一类内存问题、边界在哪里、如何安全落地、如何验证收益。
...
关于 IL2CPP 与 GC 的关系，需要谨慎处理。更准确的表述是：Unity 的托管运行时和 GC 实现会随版本、平台和脚本后端变化而有具体差异；但从开发者角度看，IL2CPP 不改变托管对象需要 GC 管理的事实，也不让 GC Alloc 自动归零。写出高频分配代码，在 IL2CPP 下仍然会分配；在 Mono 下无分配的代码，在 IL2CPP 下也不应凭空产生同等意义上的托管分配，除非涉及平台、API 或编译差异。
...
高级 GC 治理的设计重点，是把托管堆优化、资源生命周期、Native 内存、数据布局和团队流程分层处理。ScriptableObject、静态缓存、对象池、IL2CPP、Job、Burst、NativeContainer 和 Addressables 并不是同一层工具；它们分别改变配置复用、对象存活范围、运行后端、计算数据布局、资源引用计数和内存 owner。只有先明确问题属于哪一层，再选择工具，优化才不会从一个隐性成本转移成另一个隐性风险。
...
如果把 Unity GC 高级治理浓缩成一句话，那就是：GC 优化的本质不是害怕内存分配，而是尊重生命周期。知道对象从哪里来、被谁持有、什么时候释放、以什么形式存储、在什么线程计算、在什么阶段加载和卸载，项目才会真正稳定。高级 GC 技术只是工具，生命周期意识才是核心。

## 知识缺口

1. Unity 的 GC、IL2CPP、增量 GC、.NET 运行时支持、Burst 和 Job System 能力会随 Unity 版本演进，具体行为需要按项目版本和目标平台复测。
2. Addressables、AssetBundle、YooAsset 或自研资源系统的引用计数和释放规则不同，资源生命周期规范需要结合项目框架补充。
3. Job、Burst、NativeContainer 或 DOTS/ECS 的收益高度依赖数据规模、访问模式和团队维护能力，是否迁移应以性能验证和工程成本评估为准。
...
- **版本：** v2.0
- **分类：** 内存管理
- **标签：** Unity, C#, 性能优化, 内存管理, 工程治理, 响应式编程, 架构设计, 热更新, UniRx, ECS, Job System, ScriptableObject, GC, 性能分析, 托管堆, 资源管理, 代码生成, 异步加载, 对象池, 工程实践, 生命周期
