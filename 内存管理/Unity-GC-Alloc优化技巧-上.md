...
本文面向 Unity 项目中的托管内存分配优化，系统梳理 GC Alloc 的常见来源、风险边界与工程化治理方法。GC Alloc 本质上不是“错误”，而是托管对象在堆上的分配行为；真正的问题通常发生在高频路径、生命周期复杂的系统、低端设备 CPU 预算紧张的场景，以及长期运行后堆增长与 GC 触发频率失控的项目中。正式的 GC Alloc 治理必须避免两个极端：一是把所有分配都视为必须清除的坏味道，二是只在卡顿出现后临时搜索几个热点。更稳健的做法，是先区分调用频率、生命周期和设备预算，再决定缓存、复用、池化、NonAlloc API 或结构改造是否值得引入。
...
前面的分析说明，GC Alloc 优化首先是一种分类治理：热路径持续分配、低频加载分配、交互分配和泄漏型增长，不能使用同一套处理方式。设计层面应先建立性能基线、分配分类、所有权边界和代码规范，再决定具体替换手段。否则，优化很容易从性能工程变成局部代码洁癖。
...
GC Alloc 优化是 Unity 性能优化中非常重要的一环，但它绝不是简单的“看到 new 就删掉”。可靠的 GC Alloc 优化文档，必须同时覆盖分配来源、调用频率、生命周期、工具定位、团队规范和工程权衡。字符串拼接的主要问题是创建新字符串和中间对象，不应简单概括为所有情况都必然装箱；`StringBuilder.ToString()` 仍然会产生最终字符串；使用 NonAlloc API 时不能在调用前后引入新的数组转换；对象池也不是普遍最优方案，它会增加常驻内存、生命周期管理和状态残留风险。

装箱、字符串、集合、Unity API、协程、闭包、LINQ、日志、UI 列表、网络消息等问题，都不应孤立处理，而应结合 Profiler 数据、调用频率和目标设备来判断优先级。很多 GC 问题也不是分配太多，而是对象仍然可达，无法被回收。事件未解绑、闭包捕获、协程未停止、对象池未清状态、集合池逃逸，都会让内存问题变得更隐蔽。

成熟项目不应只靠开发者自觉避免 GC Alloc，而应把关键场景的分配预算、Profiler 检查、代码评审清单和自动化回归纳入研发流程。
...
1. Unity 不同版本、Mono/IL2CPP 后端和 .NET API 兼容级别会影响字符串、泛型、Span、协程等待对象和部分 Unity API 的实际分配行为，需要按项目版本与目标平台实测。
2. NonAlloc API、List 重载、Camera 与 Physics 相关接口在不同 Unity 版本中可能存在推荐用法差异，团队规范应随引擎升级复查。
3. 对象池、集合池和数字字符串缓存的收益与常驻内存成本，需要结合目标设备、峰值数量和真实场景压力测试确定。
...
- **版本：** v2.0
- **分类：** 内存管理
- **标签：** Unity, C#, 性能优化, 内存管理, 异步编程, 工程治理, 响应式编程, UniRx, GC, 缓存策略, 协程优化, 性能分析, Memory Profiler, 托管堆, 自动回收, 字符串, 字符串优化, 对象池, 集合复用, 工程实践, 生命周期
- **来源简注：** 基于 Unity GC Alloc 优化技巧主题重新编写，聚焦托管分配来源、热路径治理、生命周期边界和工程化回归。
