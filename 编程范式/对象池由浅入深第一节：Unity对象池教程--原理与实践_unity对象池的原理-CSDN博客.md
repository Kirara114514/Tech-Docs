...
## 摘要
 ### 1. 对象池原理：为什么要使用对象池？ 在游戏开发中，频繁创建和销毁对象会带来明显的性能开销。每次生成一个 **GameObject** 不仅需要分配内存，还可能触发垃圾回收（GC）和磁盘I/O，而频繁销毁对象则容易导致内存碎片。这些问题会加重CPU负担，引发帧率波动甚至卡顿。**对象池（Object Pool）** 是一种常用的优化技术，核心思想是预先创建一批对象缓存起来，在需要...
