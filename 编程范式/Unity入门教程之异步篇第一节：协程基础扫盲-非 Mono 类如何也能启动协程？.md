...
## 摘要
  初学 Unity 的小伙伴可能都会遇到这样一个疑惑：**在普通的 C# 类里，为什么不能直接使用协程？** 我记得自己第一次尝试在非 MonoBehaviour 的类里调用 `StartCoroutine` 时，要么编译报错，要么啥也不发生。当时我一脸懵逼：明明协程这么好用，为什么非得在 MonoBehaviour 脚本里才能启动呢？ 别急，今天就来聊聊 Unity 协程。我们会一起看看...
