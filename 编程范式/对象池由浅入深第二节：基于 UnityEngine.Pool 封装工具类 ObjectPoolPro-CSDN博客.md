...
## 摘要
 ### 引言：传统对象复用方式的问题 在游戏开发中，我们经常需要频繁生成和销毁一些游戏对象，例如子弹、敌人、特效等。如果每次都通过 `Instantiate` 创建和 `Destroy` 销毁，将产生较大的**性能开销**和**垃圾回收负担**。为了优化性能，很多开发者会使用**对象池（Object Pool）** 技术：预先创建一批对象，反复利用它们，用激活和隐藏来替代创建和销毁。典型做...
